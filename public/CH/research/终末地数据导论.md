...
由于文档渲染和篇幅限制，无法显示**+4%**文本的蓝色高亮，这部分内容实际上受到`<@ba.vup></>`标签控制，这些标签的说明在`HyperlinkTextTable.json`和`RichTextStyleTable.json`中，AKEData将其转为了更方便阅读的[`hyperlink.json`](https://github.com/NagiYume/AKEDatabase/blob/main/theme/hyperlink.json)和[`textstyle.json`](https://github.com/NagiYume/AKEDatabase/blob/main/theme/textstyle.json)
