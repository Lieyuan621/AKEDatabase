...
1. [伤害削减系统](#1-伤害削减系统)
2. [属性修改系统](#2-属性修改系统)
3. [异常积蓄抗性系统 (Inflict Stack Resist)](#3-异常积蓄抗性系统)
4. [周期性异常免疫系统 (Periodic Inflict Resist)](#4-周期性异常免疫系统)
5. [治疗反射至敌人系统 (Heal Reflect)](#5-治疗反射至敌人系统)
6. [受控回血系统 (Heal Under Control)](#6-受控回血系统)
7. [角色冰冻系统 (FrozenOnChar)](#7-角色冰冻系统)
8. [结晶异常系统 (Cryst Inflict)](#8-结晶异常系统)
9. [消耗异常 / 特殊 CC0 系统](#9-消耗异常--特殊-cc0-系统)
10. [移速提升 / 伤害限制系统](#10-移速提升--伤害限制系统)
11. [异常转霸体系统 (Abnormal to SuperArmor)](#11-异常转霸体系统)
...
| `CheckSpellInflictionType` | 检查法术异常类型（Fire / Pulse / Cryst / Natural） |
| `SimpleCalcBBAction` | 黑板数值计算 |
| `ModifyDynamicBlackboard` | 修改动态黑板值 |
| `SaveTargetDistanceAction` | 保存目标间距离到黑板 |
| `CompareFloat` | 浮点数比较 |

### DamageDecorateMask 掩码枚举

伤害装饰掩码（`DamageDecorateMask`）用于标识伤害来源的技能类型，`CheckDamageDecorateMask` 条件通过位运算匹配伤害类型。

| 枚举名 | 值 | 二进制 | 说明 |
|--------|-----|--------|------|
| `None` | 0 | 0 | 无标记 |
| `PoiseBreak` | 1 | 1 | 韧性击破 |
| `IgnoreGuardDodge` | 2 | 10 | 无视防御闪避 |
| `PowerAttack` | 4 | 100 | 强力攻击 |
| `FireAbnormalInitial` | 8 | 1000 | 火异常初始 |
| `PulseAbnormalInitial` | 16 | 10000 | 脉冲异常初始 |
| `CrystAbnormalInitial` | 32 | 100000 | 结晶异常初始 |
| `IgniteByMud` | 64 | 1000000 | 泥浆点燃 |
| **`NormalAttack`** | **128** | **10000000** | **普通攻击** |
| **`NormalSkill`** | **256** | **100000000** | **普通技能** |
| **`UltimateSkill`** | **512** | **1000000000** | **终极技能** |
| `PlungingAttack` | 1024 | 10000000000 | 下落攻击 |
| `Bomb` | 2048 | 100000000000 | 炸弹 |
| `CanBreakWeakness` | 4096 | 1000000000000 | 可击破弱点 |
| **`ComboSkill`** | **8192** | **10000000000000** | **连携技能** |
| `Crush` | 16384 | 100000000000000 | 碾压 |
| `Airborne` | 32768 | 1000000000000000 | 浮空 |
| `KnockDown` | 65536 | 10000000000000000 | 击倒 |
| `DashAttack` | 131072 | 100000000000000000 | 冲刺攻击 |
| `NormalAttackDamageSet` | 132228 | - | 普通攻击伤害集（组合值） |
| `IgnoreSkillImmune` | 524288 | - | 无视技能免疫 |
| `IgnoreDodgeImmune` | 786432 | - | 无视闪避免疫 |
| `NaturalAbnormalInitial` | 1048576 | - | 自然异常初始 |
| `SpellAbnormalInitial` | 1048632 | - | 法术异常初始（组合值） |
| `NormalAttackLastCombo` | 2097152 | - | 普通攻击最后一段 |
| `FireBurst` | 4194304 | - | 火爆发 |
| `CrystBurst` | 8388608 | - | 结晶爆发 |
| `PulseBurst` | 16777216 | - | 脉冲爆发 |
| `NaturalBurst` | 33554432 | - | 自然爆发 |
| `Burst` | 62914560 | - | 爆发（组合值） |
| `Burning` | 67108864 | - | 燃烧 |
| `Shatter` | 134217728 | - | 碎裂 |
| `IgniteDamageSet` | 265289784 | - | 点燃伤害集（组合值） |
| `Dot` | 268435456 | - | 持续伤害 |
| `RemainArea` | 536870912 | - | 残留区域 |
| `Fracture` | 1073741824 | - | 断裂 |
| `PhysicalInfliction` | 1073856512 | - | 物理异常（组合值） |

#### CheckDamageDecorateMask 的 checkType 模式

| checkType | 说明 | 公式 |
|-----------|------|------|
| `HasAll` | 伤害掩码必须**包含**指定的所有位 | `(damageMask & mask) == mask` |
| `HasAny` | 伤害掩码只需**包含**指定的任一位 | `(damageMask & mask) != 0` |
| `ExceptAny` | 伤害掩码**不包含**指定的任何位 | `(damageMask & mask) == 0` |

#### 危机合约中使用的掩码组合

| 掩码值 | 分解 | 语义 |
|--------|------|------|
| `128` | `NormalAttack` | 仅普通攻击 |
| `256` | `NormalSkill` | 仅普通技能 |
| `512` | `UltimateSkill` | 仅终极技能 |
| `8192` | `ComboSkill` | 仅连携技能 |
| `9088` | `ComboSkill \| UltimateSkill \| NormalSkill \| NormalAttack` | 所有主动技能类型 |

### 属性修改器 (attributeModifier) 详解

属性修改器通过 `modifyAttributeType` + `attributeType` + `formulaItem` 三元组精确定义修改方式。

#### modifyAttributeType（修改目标类型）

| 类型 | 说明 |
|------|------|
| `Specific` | 修改指定的单一属性 |
| `Main` | 修改主角色的指定属性 |
| `Composite` | 修改组合属性（由多个基础属性派生） |
...
#### attributeType（属性类型）速查表

以下为危机合约 Buff 中实际使用到的属性：

| attributeType | 中文名 | ATTR ID | 说明 |
|---------------|--------|---------|------|
| `MaxHp` | 最大生命值 | 1 | 生命值上限 |
| `MaxPoise` | 失衡值上限 | 20 | 韧性/失衡条上限 |
| `MaxUltimateSp` | 终结技能量上限 | 22 | 终极技能所需 SP |
| `MoveSpeedScalar` | 移动速度系数 | 13 | 地面移动速度 |
| `InAirMoveSpeedScalar` | 空中移速系数 | 91 | 空中移动速度 |
| `UltimateSpGainScalar` | 终结技充能效率 | 44 | SP 获取速率 |
| `ComboSkillCooldownFinalAddition` | 连携技冷却时间增加 | 47 | 连携技能冷却修正 |
| `ComboSkillCooldownScalar` | 连携技冷却时间系数 | 47 | 连携技能冷却乘算 |
| `PhysicalDamageIncrease` | 物理伤害加成 | 50 | 物理伤害增幅 |
| `FireDamageIncrease` | 灼热伤害加成 | 51 | 火元素伤害增幅 |
| `PulseDamageIncrease` | 电磁伤害加成 | 52 | 脉冲元素伤害增幅 |
| `CrystDamageIncrease` | 寒冷伤害加成 | 53 | 结晶元素伤害增幅 |
| `NaturalDamageIncrease` | 自然伤害加成 | 54 | 自然元素伤害增幅 |
| `EtherDamageIncrease` | 超域伤害加成 | 55 | 超域元素伤害增幅 |

#### globalModifier（全局修改器）

| type | 中文名 | 说明 |
|------|--------|------|
| `DashRecover` | 冲刺恢复 | 修改冲刺体力恢复速率 |
...
| 效果 | 施加异常后对应元素伤害 **-10%**，持续 10 秒 |

根据异常元素类型创建对应 Buff：
- `buff_cc_chr_phy_dmg_down`（物理）
- `buff_cc_chr_fire_dmg_down`（火）
- `buff_cc_chr_pulse_dmg_down`（脉冲）
- `buff_cc_chr_cryst_dmg_down`（结晶）
...
| 效果 | 当敌人被施加异常时，创建对应元素的抗性层 |

当检测到敌人被施加带有 `NoGuard` 标签的 Buff 时，根据异常元素类型创建对应的抗性 Buff。
...
| Buff ID | 参数 | 效果 |
|---------|------|------|
| `buff_cc_enemy_inflict_stack_resist_fire` | `dmg_scale = -0.1` | 火元素伤害 **-10%** / 层 |
| `buff_cc_enemy_inflict_stack_resist_pulse` | `dmg_scale = -0.1` | 脉冲元素伤害 **-10%** / 层 |
| `buff_cc_enemy_inflict_stack_resist_cryst` | `dmg_scale = -0.1` | 结晶元素伤害 **-10%** / 层 |
| `buff_cc_enemy_inflict_stack_resist_natural` | `dmg_scale = -0.1` | 自然元素伤害 **-10%** / 层 |
...
| 属性 | 值 |
|------|-----|
| 触发 | `OnEnemyAfterTakeSpellInfliction`（受到法术异常后） |
| 参数 | `duration = 5.0` |
| 效果 | 受到法术异常后获得对应元素 **5 秒** 免疫 |

实现方式：
1. 监听 `OnEnemyAfterTakeSpellInfliction` 事件
2. 通过 `IfElseAction` 链式判断异常类型（Fire → Pulse → Cryst → Natural）
3. 创建对应元素的免疫 Buff
...
**`OnAddedBuff` 事件组：**
1. **冰冻检测**：检查目标是否有 `Frozen` 标签，且霸体 ≤ 20 → 创建 1 层 stack
2. **物理控制检测**：检查 `AirborneStatus` / `KnockdownStatus`，且霸体 < 30 → 创建 1 层 stack
3. **特定 Buff 检测**：检查 `buff_common_originum_frozen`，且霸体 ≤ 20 → 创建 1 层 stack
4. **唐唐终极技能检测**：检查 `buff_chr_0027_tangtang_ultskill_debuff` → 创建 1 层 stack
...
├── buff_cc_chr_fire_skill_clear_frozenonchar (火技能解除)
├── buff_cc_chr_pulse_skill_clear_frozenonchar (脉冲技能解除)
...
| `buff_cc_chr_fire_skill_clear_frozenonchar` | 施放火技能 | 移除冰冻状态 |
| `buff_cc_chr_pulse_skill_clear_frozenonchar` | 施放脉冲技能 | 移除冰冻状态 |
...
实现方式：
1. 检测到结晶异常 Buff 被添加时，通过 `attributeModifier` 修改移动速度（2 条）：
...
| 效果 | 非主动技能伤害 **+200%**，普通技能伤害 **-60%**（双条目分别生效） |

实现方式：包含两个独立的 `damageModifier` 条目：
1. **条件1**（`ExceptAny`, mask=9088）：对**不包含**任何主动技能类型（普通攻击/普通技能/终极技能/连携技能）的伤害施加 `dmg_up=2.0`（+200%），覆盖 Dot、爆发、残留区域等特殊伤害
...
├── buff_cc_eny_abnormal_to_superarmor_instance (霸体实例)
...
2. 检查当前异常 Buff 数量是否为 0（即这是第一个异常）
3. 如果是，先移除已有的 `do_finish` Buff
4. 创建 `instance` Buff（霸体状态）

**`OnFinishedBuff` 事件组：**
1. 当 `SpellStatus` 或 `PhysicalStatus` Buff 结束时
2. 检查剩余异常 Buff 数量是否为 0
3. 如果是，创建 `do_finish` Buff 清理霸体状态
...
| # | 文件名 | 分类 |
|---|--------|------|
| 1 | `buff_cc_chr_combo_skill_dmg_down` | 伤害削减 |
| 2 | `buff_cc_chr_normal_attack_dmg_down` | 伤害削减 |
| 3 | `buff_cc_chr_normal_skill_dmg_down` | 伤害削减 |
| 4 | `buff_cc_chr_teammate_normal_attack_dmg_down` | 伤害削减 |
| 5 | `buff_cc_chr_teammate_take_dmg_down` | 伤害削减 |
| 6 | `buff_cc_chr_main_dmg_taken_up` | 伤害削减 |
| 7 | `buff_cc_chr_ult_dmg_down_gradual` | 伤害削减 |
| 8 | `buff_cc_chr_ult_dmg_down_gradual_stack` | 伤害削减 |
| 9 | `buff_cc_chr_ult_dmg_down_gradual_instance` | 伤害削减 |
| 10 | `buff_cc_chr_repeat_profession_dmg_down` | 伤害削减 |
| 11 | `buff_cc_chr_repeat_profession_dmg_down_instance` | 伤害削减 |
| 12 | `buff_cc_chr_dmg_down_after_inflict` | 伤害削减 |
| 13 | `buff_cc_chr_dmg_down_after_consume` | 伤害削减 |
| 14 | `buff_cc_chr_dmg_down_after_consume_instance` | 伤害削减 |
| 15 | `buff_cc_chr_phy_dmg_down` | 伤害削减 |
| 16 | `buff_cc_chr_fire_dmg_down` | 伤害削减 |
| 17 | `buff_cc_chr_pulse_dmg_down` | 伤害削减 |
| 18 | `buff_cc_chr_cryst_dmg_down` | 伤害削减 |
| 19 | `buff_cc_chr_natural_dmg_down` | 伤害削减 |
| 20 | `buff_cc_chr_dmg_down_by_distance` | 伤害削减 |
| 21 | `buff_cc_chr_dmg_reduce_maxhp` | 伤害削减 |
| 22 | `buff_cc_chr_dmg_reduce_maxhp_instance` | 伤害削减 |
| 23 | `buff_cc_chr_usp_speed_down` | 属性修改 |
| 24 | `buff_cc_chr_ult_sp_cost_increase` | 属性修改 |
| 25 | `buff_cc_chr_ult_sp_cost_increase_instance` | 属性修改 |
| 26 | `buff_cc_chr_combo_cd_up` | 属性修改 |
| 27 | `buff_cc_chr_main_attribute_down` | 属性修改 |
| 28 | `buff_cc_chr_mute_evade` | 属性修改 |
| 29 | `buff_cc_chr_normal_skill_global_cd` | 属性修改 |
| 30 | `buff_cc_chr_normal_skill_global_cd_instance` | 属性修改 |
| 31 | `buff_cc_chr_dash_recover_speed_down` | 属性修改 |
| 32 | `buff_cc_enemy_poise_up` | 属性修改 |
| 33 | `buff_cc_enemy_common_hp_up` | 属性修改 |
| 34 | `buff_cc_enemy_common_dmg_up` | 属性修改 |
| 35 | `buff_cc_enemy_inflict_stack_resist` | 异常积蓄抗性 |
| 36 | `buff_cc_enemy_inflict_stack_resist_add_listener` | 异常积蓄抗性 |
| 37 | `buff_cc_enemy_inflict_stack_resist_consume_listener` | 异常积蓄抗性 |
| 38 | `buff_cc_enemy_inflict_stack_resist_consume_delay` | 异常积蓄抗性 |
| 39 | `buff_cc_enemy_inflict_stack_resist_fire` | 异常积蓄抗性 |
| 40 | `buff_cc_enemy_inflict_stack_resist_pulse` | 异常积蓄抗性 |
| 41 | `buff_cc_enemy_inflict_stack_resist_cryst` | 异常积蓄抗性 |
| 42 | `buff_cc_enemy_inflict_stack_resist_natural` | 异常积蓄抗性 |
| 43 | `buff_cc_enemy_inflict_stack_resist_phy` | 异常积蓄抗性 |
| 44 | `buff_cc_enemy_periodic_inflict_resist` | 周期性免疫 |
| 45 | `buff_cc_enemy_periodic_inflict_resist_instance` | 周期性免疫 |
| 46 | `buff_cc_enemy_periodic_inflict_resist_fire` | 周期性免疫 |
| 47 | `buff_cc_enemy_periodic_inflict_resist_pulse` | 周期性免疫 |
| 48 | `buff_cc_enemy_periodic_inflict_resist_cryst` | 周期性免疫 |
| 49 | `buff_cc_enemy_periodic_inflict_resist_natural` | 周期性免疫 |
| 50 | `buff_cc_enemy_periodic_inflict_resist_phy` | 周期性免疫 |
| 51 | `buff_cc_chr_heal_reflect_to_eny` | 治疗反射 |
| 52 | `buff_cc_chr_heal_reflect_to_eny_heal` | 治疗反射 |
| 53 | `buff_cc_chr_heal_reflect_to_eny_shield` | 治疗反射 |
| 54 | `buff_cc_chr_heal_reflect_to_eny_stack` | 治疗反射 |
| 55 | `buff_cc_chr_heal_reflect_to_eny_stack_heal` | 治疗反射 |
| 56 | `buff_cc_chr_heal_reflect_to_eny_stack_heal_do` | 治疗反射 |
| 57 | `buff_cc_chr_heal_reflect_to_eny_stack_shield` | 治疗反射 |
| 58 | `buff_cc_chr_heal_reflect_to_eny_effect` | 治疗反射 |
| 59 | `buff_cc_enemy_heal_under_control` | 受控回血 |
| 60 | `buff_cc_enemy_heal_under_control_stack` | 受控回血 |
| 61 | `buff_cc_enemy_heal_under_control_instance` | 受控回血 |
| 62 | `buff_cc_enemy_heal_under_control_timer` | 受控回血 |
| 63 | `buff_cc_enemy_heal_on_finish` | 受控回血 |
| 64 | `buff_cc_enemy_heal_not_take_damage` | 脱战回血 |
| 65 | `buff_cc_enemy_heal_not_take_damage_instance` | 脱战回血 |
| 66 | `buff_cc_enemy_heal_not_take_damage_countdown` | 脱战回血 |
| 67 | `buff_cc_chr_frozenonchar_extend` | 角色冰冻 |
| 68 | `buff_cc_chr_frozenonchar_extend_instance` | 角色冰冻 |
| 69 | `buff_cc_chr_phy_skill_clear_frozenonchar` | 角色冰冻 |
| 70 | `buff_cc_chr_fire_skill_clear_frozenonchar` | 角色冰冻 |
| 71 | `buff_cc_chr_pulse_skill_clear_frozenonchar` | 角色冰冻 |
| 72 | `buff_cc_chr_natural_skill_clear_frozenonchar` | 角色冰冻 |
| 73 | `buff_cc_chr_cryst_inflict_to_slowdown` | 结晶异常 |
| 74 | `buff_cc_chr_normal_skill_cryst_inflict` | 结晶异常 |
| 75 | `buff_cc_chr_normal_skill_cryst_inflict_stack` | 结晶异常 |
| 76 | `buff_cc_chr_combo_skill_cryst_inflict` | 结晶异常 |
| 77 | `buff_cc_chr_combo_skill_cryst_inflict_stack` | 结晶异常 |
| 78 | `buff_cc_enemy_cryst_inflict_to_frozen` | 结晶异常 |
| 79 | `buff_cc_chr_cryst_inflict_extend` | 结晶异常 |
| 80 | `buff_cc_chr_consume_inflict_special_cc0` | 消耗异常/CC0 |
| 81 | `buff_cc_chr_consume_inflict_special_cc0_stack` | 消耗异常/CC0 |
| 82 | `buff_cc_chr_consume_inflict_special_cc0_instance` | 消耗异常/CC0 |
| 83 | `buff_cc_chr_normal_special_cc0` | 消耗异常/CC0 |
| 84 | `buff_cc_chr_combo_special_cc0` | 消耗异常/CC0 |
| 85 | `buff_cc_chr_physical_and_inflict_enhance_special_cc0` | 消耗异常/CC0 |
| 86 | `buff_cc_enemy_common_movespeedup` | 移速/伤害限制 |
| 87 | `buff_cc_enemy_common_movespeedup_dmg_limit_base` | 移速/伤害限制 |
| 88 | `buff_cc_enemy_common_movespeedup_dmg_limit_instance` | 移速/伤害限制 |
| 89 | `buff_cc_eny_abnormal_to_superarmor` | 异常转霸体 |
| 90 | `buff_cc_eny_abnormal_to_superarmor_instance` | 异常转霸体 |
| 91 | `buff_cc_eny_abnormal_to_superarmor_do_finish` | 异常转霸体 |
| 92 | `buff_cc_level_mute_switch_player_info` | 禁用/限制 |
| 93 | `buff_cc_level_mute_switch_player_info_pre` | 禁用/限制 |
| 94 | `buff_cc_enemy_death_ground_area` | 死亡地面 |
| 95 | `buff_cc_enemy_death_ground_area_dmg` | 死亡地面 |
| 96 | `buff_cc_chr_inflict_after_spell_status` | 附加异常 |
