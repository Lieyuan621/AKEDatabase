...
`1.2.11` 完成前端二级目录与公共界面组件重构。角色、武器、敌人、装备、物品、副本、活动、奖章、商店、档案库、Baker、研究、任务、危机合约等模块现在共用统一的目录项结构、选中状态、数量与版本改动标记，并统一搜索、筛选和移动端目录交互。

本版本引入 `AKEUI` 公共组件层，集中提供按钮、输入框、选择器、开关、筛选器、卡片、状态页和表格等基础控件，减少各模块重复的 DOM 构建与样式实现。全局设置与网站公告弹窗也重新设计，三套主题和桌面/移动端布局保持一致；通用表格的表头和内容默认居中显示。

此次重构主要调整前端界面与代码组织，不改变游戏数据来源和查询逻辑。既有模块深链接、筛选条件、浏览状态恢复和版本差异标记继续保留。
