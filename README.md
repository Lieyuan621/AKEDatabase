...
`1.2.7` 完善副本与活动信息展示。副本详情现在会在首通奖励之外，列出消耗理智可重复获得的固定奖励和随机奖励；活动总览时间轴不再按整天对齐，而是根据活动实际开始与结束时刻精确放置活动块。

角色总览卡片和左侧栏新增属性与职业图标，属性颜色按游戏视觉重新校准，职业图标按角色职业编号读取对应资源。角色与武器总览移除星级标签，敌人总览移除危险度标签，使卡片信息更集中；原有筛选、排序和详情数据保持不变。

富文本内嵌图标、术语链接图标及链接浮窗图标统一通过当前数据源解析完整地址，修复 `termicon` 等资源缺少 `data.akedata.wiki` 主域名或被拼接为 `//public/...` 的请求错误。
...
武器申领商品展示对应卡池内容：六星武器默认展开、其余星级折叠，每把武器显示图标与权重值；商品图标自动使用卡池内权重最高的六星武器。折扣以 `-xx%`（剩余百分比）格式显示。

版本间 diff 沿用现有架构：使用 Latest 时自动加载上一游戏版本的商店表数据，新增商店组和商品置顶并标记绿色"新增"角标，修改标记受全局设置中"显示已修改的版本间差异"控制。若旧版缺失某表，数据层自动返回空对象保证比对正常进行。

本此更新还包含：`v3-table-data.js` 的 `table()` 对当前和基线版本均添加 404 容错返回空对象，适用于所有模块；ID 字段在隐藏模式下被 CSS 规则统一隐藏；商品稀有度标识、分商店标签页的新增标记以及左侧栏新增角标的样式与位置同步调整。
