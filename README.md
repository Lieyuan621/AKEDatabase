...
- 页面标题会随当前模块和条目同步更新，浏览器标签可直接显示正在查看的内容。
- 全局设置新增“在新标签页打开条目”，默认开启；起始页条目和跨模块链接遵循该设置，左侧目录仍在当前页面切换。
- 角色推荐武器、活动与任务奖励、养成材料、技能、Buff、敌人、副本、商店、地区等关联内容支持直接跳转。
- 条目链接沿用所在组件的正常文字样式；副本、危机演算和战争回响中的怪物仅点击头像时跳转，不影响属性详情和弹层操作。

#### 角色与档案语音

- 角色和档案语音支持中文、日文、英文、韩文切换，角色语言选择位于语音记录区域上方。
- 语音资源按多个镜像站点依次回退，提高播放可用性。

#### 奖章

- 修复奖章镀层条件、镀层图标、镀层详情与进度显示，可正确查看“枯枝忿怒，复归凋零”等奖章的镀层要求。

#### 数据展示与维护

- 地区建设与“提弗洛斯的靶场”在隐藏模式关闭时不再暴露内部 ID，缺失关联使用可识别的界面文案。
- 调整部分模块顺序，并更新赞助名单。
