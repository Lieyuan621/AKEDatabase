(function () {
    ...
    let searchTerm = '';
    const showTechnicalIds = () => window.akeData?.getConfig?.().showHidden === true;
    ...
    const itemIcon = (id, itemTable) => {
        const iconId = itemTable?.[id]?.iconId || (showTechnicalIds() ? id : '');
        return iconId ? `${IMAGE_BASE_PATH}assets/beyond/dynamicassets/gameplay/ui/sprites/itemiconbig/${iconId}.png` : '';
    };
    const itemName = (id, itemTable) => itemTable?.[id]?.name?.text || (showTechnicalIds() ? id : t('unknownItem', null, '未命名物品'));
    ...
    function renderItems(items, itemTable, empty = '-') {
        ...
        return items.map(item => {
            const name = itemName(item.id, itemTable);
            return window.AKEUI.entryLinkHtml({ plugin: 'v3_item', id: item.id, label: name, className: 'ake-ui-badge', contentHtml: `<img class="ake-ui-inline-icon" src="${itemIcon(item.id, itemTable)}" alt="">${escapeHtml(name)}${item.count == null ? '' : ` × ${formatNumber(item.count)}`}` });
        }).join(' ');
    }

    function renderItemLines(items, itemTable, empty = '-') {
        if (!items?.length) return empty;
        return `<div class="region-cell-list">${items.map(item => `<div>${renderItems([item], itemTable)}</div>`).join('')}</div>`;
    }
    ...
    function describeDomainLevel(domain, row, previous, levelNames) {
        ...
        Object.entries(row.domainDevelopmentLevelEffect || {}).forEach(([levelId, effect]) => {
            ...
            const name = levelNames[levelId] || (showTechnicalIds() ? levelId : t('unknownFacility', null, '未命名设施'));
    ...
    function buildDomain(domain, mineEntries, currentVersion, tables) {
        ...
        const levelNames = Object.fromEntries((domain.levelGroup || []).map(levelId => [levelId, tables.levelDescTable?.[levelId]?.showName?.text || (showTechnicalIds() ? levelId : t('unknownFacility', null, '未命名设施'))]));
        ...
        return { ...domain, name: window.AKEV3.text(domain.domainName, showTechnicalIds() ? domain.domainId : t('unknownRegion', null, '未命名地区')), itemTable: tables.itemTable, mines: Array.from(mines.values()), levels, levelNames, archery: buildArcheryData(domain, tables) };
    }

    function domainListItem(domain, active) {
        return window.AKEUI.directoryItem({
            layout: 'entity', title: domain.name, id: showTechnicalIds() ? domain.domainId : undefined,
            icon: { src: '/public/images/assets/beyond/dynamicassets/gameplay/ui/sprites/mainhud/btn_Domain.png', alt: '' },
            active, attributes: { 'data-region-id': domain.domainId }, onSelect: () => selectDomain(domain.domainId, true)
        });
    }
    ...
    function renderLists() {
        ...
        if (activeDomainId && !filtered.some(domain => domain.domainId === activeDomainId)) selectDomain(filtered[0]?.domainId || null, false);
    }
    ...
    function renderLevelOutput(domain) {
        const rows = domain.levels.map(level => {
            const production = level.outputs.length ? `<div class="region-cell-list">${level.outputs.map(item => `<div>${window.AKEUI.entryLinkHtml({ plugin: 'v3_item', id: item.id, label: itemName(item.id, domain.itemTable), className: 'ake-ui-badge', contentHtml: `<img class="ake-ui-inline-icon" src="${itemIcon(item.id, domain.itemTable)}" alt="">${escapeHtml(itemName(item.id, domain.itemTable))} ${formatNumber(item.output)}/min` })}</div>`).join('')}</div>` : '-';
    ...
    function renderMineDetails(domain) {
        const showIds = showTechnicalIds();
        const rows = domain.mines.sort((a, b) => String(a.levelId).localeCompare(String(b.levelId)) || String(a.logicMineDataId).localeCompare(String(b.logicMineDataId))).map(mine => {
            ...
            const idCell = showIds ? `<td>${escapeHtml(mine.logicMineDataId)}${mine.isNew ? ` <span class="ake-ui-badge">${t('new', null, '新增')}</span>` : ''}${oemLink}</td>` : '';
            const mineralName = itemName(mine.itemId, domain.itemTable);
            const mineral = window.AKEUI.entryLinkHtml({ plugin: 'v3_item', id: mine.itemId, label: mineralName, contentHtml: `<img class="ake-ui-inline-icon" src="${itemIcon(mine.itemId, domain.itemTable)}" alt="">${escapeHtml(mineralName)}` });
            return `<tr>${idCell}<td>${escapeHtml(mine.levelName)}</td><td>${mineral}</td><td>${ranges || '-'}</td></tr>`;
        }).join('');
        return renderCollapsibleSection(t('mineDetails'), `<div class="ake-ui-table-wrap"><table class="ake-ui-table"><thead><tr>${showIds ? `<th>${t('mineId')}</th>` : ''}<th>${t('mineRegion', null, '所在地区')}</th><th>${t('mineral')}</th><th>${t('stageIntervals', null, '地区等级区间、产量与纯度')}</th></tr></thead><tbody>${rows}</tbody></table></div>`);
    }
    ...
    function renderSettlementSection(domain) {
        ...
        const cards = settlements.map(settlement => {
            const traits = (settlement.wantTagIdGroup || []).map(id => domain.tables.settlementTagTable?.[id]).filter(Boolean)
                .map(tag => `<span class="ake-ui-badge">${escapeHtml(tag.settlementTagName?.text || (showIds ? tag.settlementTagId : t('unknownTag', null, '未命名标签')))}${tag.desc?.text ? `：${escapeHtml(tag.desc.text)}` : ''}</span>`).join(' ');
            const levels = Object.entries(settlement.settlementLevelMap || {}).sort(([a], [b]) => Number(a) - Number(b)).map(([levelKey, level]) => {
                ...
                const tradeLines = trades.length ? `<div class="region-cell-list">${trades.map(item => {
                    const name = itemName(item.itemId, domain.itemTable);
                    return `<div>${window.AKEUI.entryLinkHtml({ plugin: 'v3_item', id: item.itemId, label: name, className: 'ake-ui-badge', contentHtml: `<img class="ake-ui-inline-icon" src="${itemIcon(item.itemId, domain.itemTable)}" alt="">${escapeHtml(name)} → ${formatNumber(item.rewardMoneyCount)} ${t('dispatchCoupon', null, '调度券')} / ${formatNumber(item.stmExp)} ${t('developmentValue', null, '发展值')}` })}</div>`;
                }).join('')}</div>` : '-';
            ...
            return `<article class="ake-ui-card" data-ake-component="card"><div class="ake-ui-card__content"><header class="ake-ui-card__header"><div class="ake-ui-card__heading"><h4 class="ake-ui-card__title">${escapeHtml(settlement.settlementName?.text || (showIds ? settlement.settlementId : t('unknownSettlement', null, '未命名据点')))}</h4><div class="ake-ui-card__badges">${traits}</div></div></header><div class="ake-ui-table-wrap"><table class="ake-ui-table region-settlement-table"><thead><tr><th>${commonT('level')}</th><th>${t('requiredDevelopment', null, '升级所需发展值')}</th><th>${t('materialExchange', null, '物资兑换调度券')}</th><th>${t('traitDescription', null, '等级说明')}</th></tr></thead><tbody>${levels}</tbody></table></div></div></article>`;
        }).join('');
    ...
    function facilityCard(title, subtitle, rows, effectTitle = null, route = null) {
        const heading = route?.id ? window.AKEUI.entryLinkHtml({ plugin: route.plugin, id: route.id, label: title, className: 'ake-ui-card__title', contentHtml: escapeHtml(title) }) : `<h4 class="ake-ui-card__title">${escapeHtml(title)}</h4>`;
        return `<article class="ake-ui-card" data-ake-component="card"><div class="ake-ui-card__content"><header class="ake-ui-card__header"><div class="ake-ui-card__heading">${heading}${subtitle ? `<div class="ake-ui-card__id">${escapeHtml(subtitle)}</div>` : ''}</div></header><div class="ake-ui-table-wrap"><table class="ake-ui-table"><thead><tr><th>${commonT('level')}</th><th>${t('upgradeCost', null, '升级消耗调度券')}</th><th>${effectTitle || t('facilityEffect', null, '说明、奖励或解锁内容')}</th></tr></thead><tbody>${rows}</tbody></table></div></div></article>`;
    }
    ...
    function renderFacilitySection(domain) {
        ...
        Object.values(tables.shopChannelTable || {}).filter(row => row.shopGroupId === domain.domainShopGroupId).forEach(channel => {
            ...
            cards.push(facilityCard(channel.channelName?.text || (showTechnicalIds() ? channel.levelId || channel.shopGroupId : t('unknownFacility', null, '未命名设施')), t('facilityTypes.shop', null, '物资调度'), rows, null, { plugin: 'v3_shop', id: channel.shopGroupId }));
        });
        Object.values(tables.domainDepotTable || {}).filter(row => row.domainId === domain.domainId).forEach(depot => {
            ...
            cards.push(facilityCard(depot.depotName?.text || (showTechnicalIds() ? depot.domainDepotId : t('unknownFacility', null, '未命名设施')), t('facilityTypes.depot', null, '仓储节点'), rows));
        });
        Object.values(tables.kiteStationTable || {}).filter(row => row.domainId === domain.domainId).forEach(station => {
            ...
            cards.push(facilityCard(station.name?.text || (showTechnicalIds() ? station.kiteStation : t('unknownFacility', null, '未命名设施')), t('facilityTypes.kite', null, '环境监测'), rows));
        });
        Object.values(tables.recycleBinTable || {}).filter(row => row.domainId === domain.domainId).forEach(bin => {
            ...
            cards.push(facilityCard(`${t('facilityTypes.recycle', null, '资源回收站')}${showTechnicalIds() ? ` #${bin.serialId}` : ''}`, showTechnicalIds() ? bin.levelId : '', rows));
        });
        Object.values(tables.sewageTable || {}).filter(row => row.domainId === domain.domainId).forEach(plant => {
            ...
            cards.push(facilityCard(plant.name?.text || (showTechnicalIds() ? plant.id : t('unknownFacility', null, '未命名设施')), t('facilityTypes.sewage', null, '净水节点'), rows));
        });
        ...
        if (domain.archery) {
            ...
            if (rows) cards.push(facilityCard(t('archery.title', null, '提弗洛斯的靶场'), showTechnicalIds() && archery.mapLevel ? t('archery.mapLevel', { level: archery.mapLevel }) : '', rows, t('levelRewards', null, '等级奖励')));
        }
    ...
    function selectDomain(id, updateUrl = false) {
        activeDomainId = id;
        renderLists();
        const domain = domains.find(item => item.domainId === id);
        if (domain) {
            renderDetail(domain);
            if (updateUrl) window.__akeRouter?.updateUrl?.('region', domain.domainId);
        }
    }
    ...
    async function load() {
        ...
        const deepDomain = deepId ? domains.find(domain => domain.domainId === deepId) : null;
        if (deepId && !deepDomain) window.__akeRouter?.onDeepLinkNotFound?.(deepId, false);
        activeDomainId = deepDomain?.domainId || domains[0]?.domainId || null;
        renderLists();
        if (activeDomainId) renderDetail(deepDomain || domains[0]);
    }
